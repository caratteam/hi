...
import { createSubagentExtension } from "./extensions/subagent/index.js";
...
function buildSystemPrompt(
	...
	skills: Skill[],
	extensionTools: Array<{ name: string; description: string }> = [],
): string {
...
${extensionTools.map((t) => `- ${t.name}: ${t.description}`).join("\n")}
...
function createRunner(
	...
	const extensionToolDescriptions = Array.from(subagentExtension.tools.values()).map((t: any) => ({
		name: t.definition.name as string,
		description: ((t.definition.description || "") as string).split("\n")[0],
	}));
	...
	const resourceLoader: ResourceLoader = {
		getExtensions: () => ({
			extensions: [skillMemoryExtension, subagentExtension],
			errors: [],
